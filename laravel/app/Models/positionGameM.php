    ...
    public function poBettingInsert($bettingData)
    {
        ...
        if ($bettingData['action'] != NULL && $bettingData['action'] == 'insert')  //判斷值是否由欄位輸入
        {
            ...
            DB::table('bs_sdBetting')->insert(array(                          //新增下注資料
                array('user_id' => $userId, 'user_name' => $userName, 'h_id' => $bettingData['h_id'], 'horse_name' => $bettingData['horseName'], 'odds' => $odds[0]->odds, 'horse_picture' => $bettingData['horsePic'], 'betting_time' => $bettingTime)
            ));
